...
local ME = {
	skipDialogs							= {
		...
		{zone = 400, pattern = "Klasse ändern", button = 2, modifier = "sCA"},	-- Hausmädchen inhouse - Klasse tauschen
		...
		{pattern = "klasse wechseln", modifier = "sCa"},												-- Hausmädchen outhouse - Klasse wechseln
		{pattern = "klasse wechseln", modifier = "sCA"},												-- Hausmädchen outhouse - Klasse tauschen
		{pattern = "Haus betreten", modifier = "scA"},													-- Hausmädchen outhouse - Haus betreten
		{pattern = "Bank öffnen", modifier = "sca"},														-- Hausmädchen outhouse - Bank öffnen
	},
}
...
local function CheckRequestDialogs(event, msg, ...)
	...
	if RB.settings.skipDialogs==true then
		...
		for k,v in pairs(ME.skipDialogs) do
			if (not v.zone and GetZoneID()~=400 and GetZoneID()~=401) or GetZoneID()==v.zone then
						...
						if not v.pattern and not v.button then
							...
							for i=start,stop do
								...
								if g_SpeakFrameData.count>=i and g_SpeakFrameData.option[i].type==56 then
									...
									if pOK and but>0 then break end
								end
							end
							RB.Debug("-->", v.pattern, start, stop, "->", but, pOK)
						end
