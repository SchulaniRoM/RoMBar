...
function LI.MiniConfigShow(relativeTo, relativePoint, point, offsetX, offsetY)

	local x, y 				= GetCursorPos()
	local xOff, yOff	= offsetX or (x / (GetUIScale() or 1)), offsetY or (y / (GetUIScale() or 1))
	local frame				= relativeTo or UIParent
	local ancR, ancP	= relativePoint or "TOPLEFT", point or "TOPLEFT"

	UIDropDownMenu_SetAnchor(LootIt_Dropdown, xOff, yOff, ancR, ancP, frame)
		...
		if not UIDROPDOWNMENU_MENU_LEVEL or UIDROPDOWNMENU_MENU_LEVEL == 1 then
			UIDropDownMenu_AddButton({
				...
				func = function()
					...
					elseif loot == LI.Trans("SELL") then
						loot = 4
          end
