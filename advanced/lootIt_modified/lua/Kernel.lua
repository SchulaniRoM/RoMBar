...
function LI.OnEvent(this, event, arg1, arg2, arg3, arg4)
	...
	elseif event == "STORE_OPEN" and LI.Active() then
		LI.RunAutoSell()
	end
end
...
function LI.RunAutoSell()

	if not LI_Data.Options.autosell then return end
	local used, total = GetBagCount()
	local cheap, max10 = {}, 0
		...
		if not letTime or letTime > 0 then
			for i = j * 30 - 29, j * 30 do
				...
				if sname and sname ~= "" then
					...
					if loot==5 then
						if LI_Data.Options.testing then
							LI.io(string.format(LI.Trans("TESTING_SELL_MESSAGE"), "|cffff0050"..link.."|r", i))
						else
							-- sell item
							max10 = max10 + 1
							UseBagItem(slot)
						end
						if max10>=10 and LI_Data.Options.autosellmax10 then
							LI.io(LI.Trans("SELL_MAX10_MESSAGE"))
							return
						end
					end
				end
